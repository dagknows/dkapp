#!/bin/bash
# DagKnows Application Version Management Setup Script
...
print_header "DagKnows Application Version Management Setup"
...
if [ ! -f "$SCRIPT_DIR/version-manager.py" ]; then
    ...
    echo "Please ensure you have the latest dkapp installation."
    exit 1
fi
...
# AWS CLI warning
print_warning "NOTE: AWS CLI is recommended for full functionality."
echo ""
echo "  AWS CLI is used to resolve 'latest' tags to specific ECR versions."
echo "  Without it, versions will be recorded as 'latest' initially."
echo ""
echo "  To configure AWS CLI:"
echo "    1. Install: sudo apt install awscli  (or pip install awscli)"
echo "    2. Configure: aws configure"
echo ""
echo "  You can set up AWS CLI later and run 'make resolve-tags' to update."
echo ""
...
if python3 "$SCRIPT_DIR/migrate-to-versioned.py"; then
    ...
    if ! command -v aws &> /dev/null; then
        print_warning "AWS CLI not found - versions recorded as 'latest'"
        echo ""
        echo "To resolve to specific version tags later:"
        echo "  1. Install and configure AWS CLI"
        echo "  2. Run: make resolve-tags"
    fi
else
